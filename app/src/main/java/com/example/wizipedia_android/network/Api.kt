...
interface Api {
    ...
    suspend fun getAllCharacter(@Path("character_name") characterName: String): Character

    @GET("/houses")
    suspend fun getAllHouses(): Houses

    @GET("/houses/{house_name}")
    suspend fun getCharactersForHouse(@Path("house_name") houseName: String)
    ...
    suspend fun getMovie(@Path("movie_name") movieName: String)
